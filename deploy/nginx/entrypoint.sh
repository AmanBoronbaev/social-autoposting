...
: "${APP_MAX_UPLOAD_BYTES:=524288000}"
...
case "$APP_MAX_UPLOAD_BYTES" in
  *[!0-9]*|'') echo "APP_MAX_UPLOAD_BYTES must be a whole number of bytes" >&2; exit 1 ;;
esac
path_body="${APP_PATH#/}"
path_body="${path_body%/}"
...
envsubst '${APP_DOMAIN} ${APP_PATH} ${APP_MAX_UPLOAD_BYTES}' < "$template" > /etc/nginx/conf.d/default.conf
